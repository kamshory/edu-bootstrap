...
function initContextMenuFile() {
  $('.row-data-file').each(function (index) {
    ...
    if (
      ...
      contextMenu(
        this,
        contextMenuListFile(filetype, filepath, fileurl, attr)
      );
    } else {
  ...
  if (filetype.indexOf('image') == 0) {
    ...
    cm = [
      ...
      },
      {
        ...
        linkurl: "javascript:previewFile('" +
          fileurl +
          "', " +
          width +
          ', ' +
    ...
    cm = [
      ...
      {
        caption: 'Play Video',
        linkurl: "javascript:playVideo('" + fileurl + "', 'html5')",
        classname: 'play',
      },
      {
        caption: 'Compress File',
    ...
    cm = [
      ...
      {
        caption: 'Compress Audio',
        linkurl: "javascript:compressAudio('" + filepath + "')",
        classname: 'play',
      },
      {
        caption: 'Compress File',
    ...
    cm = [
      ...
      {
        caption: 'Preview SVG',
        linkurl: "javascript:previewSVG('" + fileurl + "')",
        classname: 'preview',
      },
      {
      ...
      {
        caption: 'Edit Code',
        linkurl: 'code-editor.php?filepath=' +
...
function preloadImage() {
  $(
    document
  ).on('hover', '.row-data-file[data-file-type*="image"]', function () {
...
function playVideo(url, type) {
  ...
  if (videoDimensions[previewURL] != null) {
    height =
      width *
      videoDimensions[previewURL].height /
      videoDimensions[previewURL].width;
  }
  ...
  if (type == 'iframe') {
    ...
    html =
      '<video src="' +
      previewURL +
      '" poster="style/images/movie.png" controls="true" width="' +
      width +
      '" height="' +
      height +
      '"><object width="' +
      ...
      '" height="' +
      height +
      '" type="application/x-shockwave-flash" data="moxieplayer.swf"><param name="url" value="' +
      previewURL +
...
function compressAudio(path) {
  $('#common-dialog-inner').html(
    'Compress audio will reduce the quality and file size. This process can not be undone.<br />Are you sure yo want to compress this audio?'
  );
  $('#common-dialog').dialog({
    ...
    buttons: {
      Yes: function () {
        ...
        $('#common-dialog').html(
          'Please wait while audio file is being compressed.'
        );
      ...
      buttons: {
        OK: function () {
          ...
          if (nn == '') {
            ...
            jqAlert(
              'New name and old name must be different.',
              'Invalid Name'
            );
          } else {
...
function compressSelectedFile() {
  ...
  if (chk) {
    ...
    do {
      ...
      num1 = $('.file-area .row-data-dir[data-file-name="' + dir + '"]')
        .length;
...
function changePermission(
  ...
  $('#common-dialog-inner').html(
    '<table width="100%" border="0" cellspacing="1" cellpadding="2" class="table table-striped table-sm file-table permission-table">' +
    '<thead>' +
    '  <tr>' +
    '    <td>User</td>' +
    '    <td width="25%">Read</td>' +
...
function about() {
  let html =
    ...
    '<tr><td>Developer</td><td><a href="http://www.planetbiru.com" target="_blank">Kamshory Developer</a></td></tr>' +
...
function normalizeTable() {
  $('.file-result-table tbody tr').each(function (index, element) {
    $(this).attr(
      'data-file-name-lower',
      $(this).attr('data-file-name').toLowerCase()
    );
  });
}
function filterFile(name) {
  ...
  if (name == '') {
    ...
    $(
      '.file-result-table tbody tr[data-file-name-lower*="' + name + '"]'
    ).css({ display: '' });
  }
...
function initEXIF() {
  $(document).on('click', '.capture-info', function () {
    ...
    let obj = jsdata[0];
    let html =
      '<table width="100%" border="0" cellspacing="1" cellpadding="2" class="table table-striped table-sm exif-table">';
    html += '<tbody>';
    for (key in obj) {
      html +=
...
function FileSelectHandler(e) {
  ...
  for (let i in files) {
    ...
    html +=
      '<div class="file-upload-item" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">' +
      files[i].name +
      '</div>';
  }
  html += '</div>';
