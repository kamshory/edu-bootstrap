...
// Contributor Kamshory
// Fixing some issues on PHP 7

class AStoIMG //NOSONAR
{
	...
	public  $marker='none';
	public $markerfill = 'green';
	public $gridcolor = 'gray';
	public  $axescolor = 'black';
	...
	public $AScom; 
	public function __construct($w = 200, $h = 200) {
		$this->constX($w, $h);
	}

	public function constX($w = 200, $h = 200)
	{
		$this->xmin = -5; 
		$this->xmax = 5; 
		$this->ymin = -5; 
		$this->ymax = 5; 
		$this->border = array(5,5,5,5);
		$this->stroke = 'black'; 
		$this->fill = 'none'; 
		$this->curdash=''; 
		$this->isdashed=false; 
		$this->marker='none';
		$this->markerfill = 'green'; 
		$this->gridcolor = 'gray'; 
		$this->axescolor = 'black';
		$this->strokewidth = 1; 
		$this->dotradius=8; 
		$this->ticklength=4;
		$this->fontsize = 12; 
		$this->fontfill=''; $this->fontbackground='';
	...
	public function processShortScript($script) //NOSONAR
	{
		/*
		Commented
	...
	public function processScript($script) //NOSONAR
	{
		/*
		Commented
		$xmin = -5; $xmax = 5; $ymin = -5; $ymax = 5; $border = 5;
	...
	public function ASsetdash() //NOSONAR
	{
		if (func_num_args()>0) {
			$dash = func_get_arg(0);
			$this->curdash = $dash;
		...
		if ($dash=='none' || !preg_match('/\d/',$dash)) {
			...
			if (!empty($style)) {
				$styleColor = $this->createStyleColor($style);
				imagesetstyle($this->img, $styleColor);
			}
	...
	public function createStyleColor($style)
	{
		$styleColor = array();
		foreach($style as $key=>$val)
		{
			$styleColor[$key] = $this->$val;
		}
		return $styleColor;
	}
	public function asNum($raw)
	{
		return ((int) $raw);
	}
	public function AStext($arg) {
		$pos = '';  $angle = 0;
		if (func_num_args()>1) {
			$p = $this->pt2arr($arg);
			$st = func_get_arg(1);
	...
	public function AStextInternal($p,$st,$pos,$angle) //NOSONAR
	{
		/*
		Commented
		if (func_num_args()>1) {
	...
	public function ASinitPicture($arg) //NOSONAR
	{
	...
	public function ASaxes($arg) //NOSONAR
	{
		...
		if (count($arg)>5 && ($arg[5]=='off' || $arg[5]=='0') ) {
			$dox = false;
		}
		if (count($arg)>6 && ($arg[6]=='off' || $arg[6]=='0') ) {
			$doy = false;
		}
		if ($xscl<0) {
			$xscl *= -1;
		...
		$ac = $this->axescolor;
		if ($doy && $this->origin[0]>=$this->winxmin && $this->origin[0]<=$this->winxmax) {
		...
		if ($dox && $this->origin[1]>=$this->winymin && $this->origin[1]<=$this->winymax) {
	...
	public function AScircle($arg) {
		$this->ASellipse(array($arg[0],$arg[1],$arg[1]));
	}
	public function ASellipse($arg) {
		$p = $this->pt2arr($arg[0]);
	...
	public function ASarrowhead($v,$w) {
	...
	public function ASslopefield($arg) //NOSONAR
	{
		$func = $arg[0];
		if (count($arg)>1) {
			$dx = $arg[1];
	...
	public function ASplot($function) //NOSONAR
	{
	...
	public function parseargs($str) //NOSONAR
	{
		$lp = 0; $qd = 0; $bd=0; $args = array();
		for($i=0; $i<strlen($str); $i++) {
			if (substr($str, $i, 1) =='[' && $qd==0) { 
				$bd++;
			}
			if (substr($str, $i, 1) == ']' && $qd==0) { 
				$bd--;
