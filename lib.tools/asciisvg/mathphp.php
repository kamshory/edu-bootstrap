...
function mathphp($st, $varlist) //NOSONAR
{
  ...
  $st = preg_replace("/([0-9])([\(])/", "$1*$2", $st); //NOSONAR
  if ($varlist != null) {
    $st = preg_replace("/([0-9])(" . $varlist . ")/", "$1*$2", $st);
  }
  $st = preg_replace("/([0-9])(sin|cos|tan|sec|csc|cot|ln|log|exp|asin|acos|atan|sqrt|abs)/", "$1*$2", $st); //NOSONAR
  //want 4E2 to be scientific notation

  $st = preg_replace('/([0-9])\*\(exp\(1\)\)([0-9])/', "\\1E\\2", $st); //NOSONAR

  $st = preg_replace('/([0-9])\*E([\-0-9])/', "\\1E\\2", $st); //NOSONAR
  ...
  while ($i = strpos($st, "^")) {
    //find left argument
    if ($i == 0) {
      return "Error: missing argument"; //NOSONAR
    }
    $j = $i - 1;
    $ch = substr($st, $j, 1);
    if ($ch >= "0" && $ch <= "9") { // look for (decimal) number
      $j--;
      while ($j >= 0 && ($ch = substr($st, $j, 1)) >= "0" && $ch <= "9") {
        $j--;
      }
      if ($ch == ".") {
        $j--;
      ...
      while ($j >= 0 && (($ch = substr($st, $j, 1)) >= "a" && $ch <= "z" || $ch >= "A" && $ch <= "Z" || $ch == '$')) {
        ...
        return "Error: incorrect syntax in " . $st . " at position " . $j; //NOSONAR
      }
    }
    //find right argument
    if ($i == strlen($st) - 1) {
    ...
    if ($ch >= "0" && $ch <= "9" || $ch == "-" || $ch == ".") { // look for signed (decimal) number
      $k++;
      while ($k < strlen($st) && ($ch = substr($st, $k, 1)) >= "0" && $ch <= "9") {
        $k++;
      }
      if ($ch == ".") {
        $k++;
      ...
      if ($ch == '(' && preg_match('/^(sin|cos|tan|sec|csc|cot|sinh|cosh|tanh|sech|csch|coth|ln|log|exp|asin|acos|atan|asinh|acosh|atanh|sqrt|abs)$/', substr($st, $i + 1, ($k - $i - 1)))) //NOSONAR 
      {
        $nested = 1;
        $k++;
        while ($k < strlen($st) && $nested > 0) {
...
function safepow($base, $power) //NOSONAR
{
  if ($base == 0) {
    if ($power == 0) {
      return sqrt(-1);
...
function factorial($x)
{
  ...
  // Old logic : return ($x<0?false:($x==0?1:$x));
