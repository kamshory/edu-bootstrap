...
if(@$_GET['option'] == 'delete')
{
	...
	if($stmt->rowCount() > 0)
	{
		...
		$database->executeDelete($sql, true);
		header("Location: ".$picoEdu->gateBaseSelfName()."?test_id=$test_id"); //NOSONAR
	}
}
...
if(isset($_POST['savetext']) && @$_GET['option'] == 'add')
{
	...
	if($stmt->rowCount() > 0)
	{
		...
		foreach($clear_data as $question_no=>$question)
		{
			...
			{
				$content = addslashes(nl2br(utf8ToEntities(\Pico\PicoDOM::filterHtml(addImages(@$object['question'], $test_dir, $base_src)))));
				...
				else
				{
					...
					if(@is_array($object['option']) && count($object['option']))
					{
						...
						{
							$content_option = addslashes(nl2br(utf8ToEntities(\Pico\PicoDOM::filterHtml(addImages($option['text'], $test_dir, $base_src)))));
...
else if(@$_GET['option'] == 'edit')
{
	...
	if ($stmt->rowCount() > 0) 
	{
		...
		if ($stmt3->rowCount() > 0) {
			...
			<tr>
				<td>Pengacakan Pilihan</td>
				<td><label><input type="checkbox" name="random" id="random" value="1"<?php echo $picoEdu->ifMatch($data['random'], 1, \Pico\PicoConst::INPUT_CHECKBOX_CHECKED); ?> /> Diacak</label></td>
			</tr>
			</table>
			</div>
			<div class="question-editor">
		...
		<div class="button-area">
		<input type="submit" name="save" id="save" class="btn btn-success" value="Simpan" />
		<input type="button" name="add" id="add" class="btn btn-success" value="Tambah Soal" onclick="window.location='<?php echo $picoEdu->gateBaseSelfName();?>?option=add&test_id=<?php echo $test_id; ?>'" />
		<input type="button" name="showall" id="showall" class="btn btn-success" value="Tampilkan Semua Soal" onclick="window.location='<?php echo $picoEdu->gateBaseSelfName();?>?test_id=<?php echo $test_id; ?>'" />
		</div>
...
<input type="button" name="download-word" id="download-word" class="btn btn-primary" value="Download Format Word" onclick="downloadInWord()" />
