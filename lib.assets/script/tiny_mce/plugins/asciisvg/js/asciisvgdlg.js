...
var AsciisvgDialog = {
	...
	init : function() {
		...
		this.ascii_svg_server = tinyMCEPopup.getWindowArg('ascii_svg_server');
		this.alignm = tinyMCEPopup.getWindowArg('alignm');
		if (ASnoSVG) {
			document.getElementById("preview").innerHTML = '<img id="previewimg" style="width:'+this.width+'px; height: '+this.height+'px; vertical-align: middle; float: none;" src="'+ this.ascii_svg_server + '?sscr='+encodeURIComponent(this.sscr)+'" script=" " />';
		} else {
	...
	insert : function() {
		...
		if (this.isnew) {
			...
			tinyMCEPopup.editor.execCommand('mceInsertContent', false, '<img style="width:'+this.width+'px; height:'+this.height+'px; '+aligntxt+'" src="'+ this.ascii_svg_server + '?sscr='+encodeURIComponent(this.sscr)+'" data-sscr="'+this.sscr+'" script=" " />');
		} else {
			el = tinyMCEPopup.editor.selection.getNode();
			ed.dom.setAttrib(el,"data-sscr",this.sscr);
			ed.dom.setAttrib(el,"src",this.ascii_svg_server + '?sscr='+encodeURIComponent(this.sscr));
	...
	graphit : function() {
		...
		if (ASnoSVG) {
			...
			pvimg.src = this.ascii_svg_server + '?sscr='+encodeURIComponent(commands);
