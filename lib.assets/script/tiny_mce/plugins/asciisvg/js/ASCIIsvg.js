...
function mathjs(st,varlist) {
  ...
  if (varlist != null) {
	  ...
	  // 10/25/10 re-removed \( for x(1+x); moved f() handling to AMhelpers;  6/1/09 readded \( for f(350/x)
...
//	  picture = pictures[index];
	  if (!ASnoSVG) {
		  ...
		  if (picture.getAttribute("sscr")!='') {
			  ...
			  n.setAttribute("src",ascii_svg_server+'?sscr='+encodeURIComponent(picture.getAttribute("sscr")));
