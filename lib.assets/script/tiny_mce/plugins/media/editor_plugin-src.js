(function() {
    ...
    tinymce.create("tinymce.plugins.MediaPlugin", {
        init: function(n, j) {
			...
			n.onEvent.add(function(n,e) {
				if (e.type=="mouseup") {
					el = n.selection.getNode();
					if (el.nodeName == 'IMG' && el.className.indexOf("mceItemMedia") !== -1) {
						setTimeout(function() { r.processresize(n,el)},50);
					}
				}
			});
        },
		...
		processresize : function(ed,el) {
			...
			var json_data = ed.dom.getAttrib(el,"data-mce-json") || '';
			var json_obj = h.parse(json_data, "'");
			json_obj.video.attrs.width = width+'';
			json_obj.video.attrs.height = height+'';
			json_data = h.serialize(json_obj, "'");
			el.setAttribute("data-mce-json", json_data);			
		},
        ...
        dataToHtml: function(j, k) {
            var html = this.editor.serializer.serialize(this.dataToImg(j, k), {
                forced_root_block: "",
                force_absolute: k
            });
			console.log(html);
